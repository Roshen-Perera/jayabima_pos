...
export async function generateSupplierStatementPDF(data: GenerateSupplierStatementPDFParams): Promise<Buffer> {
  ...
  const docTitle = "SUPPLIER ACCOUNTS PAYABLE STATEMENT";
  const docTitleWidth = fontBold.widthOfTextAtSize(docTitle, 10);
  ...
  page.drawText("STATEMENT FOR SUPPLIER", {
    x: margin + 10,
    y: cardY + 52,
    size: 8,
    font: fontBold,
  ...
  // Tile 1: Purchases Billed
  page.drawRectangle({ x: startX, y: tileY, width: tileW, height: tileH, color: rgb(1, 1, 1), borderColor, borderWidth: 1 });
  page.drawText("PURCHASES", { x: startX + 16, y: tileY + 34, size: 7.5, font: fontBold, color: secondaryColor });
  ...
  const drawTableHeader = (p: typeof page, posY: number) => {
    ...
    p.drawText("PURCHASE (+)", { x: margin + 331, y: posY - 10, size: 8, font: fontBold, color: secondaryColor });
    p.drawText("PAYMENT (-)", { x: margin + 401, y: posY - 10, size: 8, font: fontBold, color: secondaryColor });
    p.drawText("PAYABLE BAL", { x: margin + 461, y: posY - 10, size: 8, font: fontBold, color: secondaryColor });
  };
  ...
  data.entries.forEach((entry, idx) => {
    ...
    page.drawText(creditText, { x: margin + 401, y: currentY - 8, size: 8, font: fontBold, color: greenColor });
  ...
  page.drawText("Official Supplier Accounts Payable Statement generated from Jayabima Hardware POS.", {
    x: margin,
    y: currentY - 20,
    size: 7.5,
    font: fontRegular,
  ...
  const netText = `Net Payable Balance: LKR ${data.netOutstanding.toLocaleString("en-US", { minimumFractionDigits: 2 })}`;
